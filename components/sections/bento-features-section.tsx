...
function FeatureHeader({ icon: Icon }: { icon: any }) {
  return (
    <div className="flex flex-1 w-full h-full min-h-[6rem] rounded-xl bg-gradient-to-br from-emerald-900/20 to-emerald-600/20 relative overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-r from-emerald-600/10 to-emerald-400/10" />
      <div className="flex items-center justify-center w-full h-full">
        <Icon className="w-12 h-12 text-emerald-400" />
      </div>
      <div className="absolute bottom-2 right-2 w-8 h-8 bg-emerald-500/20 rounded-full" />
      <div className="absolute top-2 left-2 w-4 h-4 bg-emerald-400/30 rounded-full" />
    </div>
  )
}
  ...
  return (
    <section className="py-20 relative overflow-hidden">
      <div className="absolute inset-0">
        ...
        <div className="absolute bottom-1/4 left-1/4 w-72 h-72 bg-emerald-400/5 rounded-full blur-3xl" />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-lg text-white/70 max-w-3xl mx-auto">
            XaffReps oferuje kompletny ekosystem narzędzi i funkcji w nowoczesnym interfejsie.
          </p>
        </motion.div>
