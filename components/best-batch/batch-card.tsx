// components/best-batch/batch-card.tsx
...
import { ShoppingCart, Heart, Eye, ImageOff, Loader2, AlertTriangle, Star } from "lucide-react";
...
interface BatchCardProps {
  ...
  viewMode: "grid" | "list";
  isFavorited: boolean;
  onToggleFavorite: (batch: any) => void;
}
...
export function BatchCard({ batch, viewMode, isFavorited, onToggleFavorite }: BatchCardProps) {
  ...
  const sellerName = batch.shopInfo?.ShopName || 'Nieznany';
  
  const BatchImage = () => (
    batch.thumbnailUrl ? (
        <Image src={batch.thumbnailUrl} alt={batch.name} width={300} height={300} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
    ) : <PlaceholderImage />
  );
      ...
      {viewMode === "list" ? (
        <motion.div whileHover={{ y: -5 }} className="glass-morphism rounded-2xl p-6 hover:bg-white/10">
          <div className="flex items-center space-x-6">
              <Link href={`/bb/${batch._id}`} className="block flex-shrink-0">
                  <div className="w-24 h-24 rounded-lg overflow-hidden bg-white/5"><BatchImage /></div>
              </Link>
              <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                      <Star className="w-4 h-4 text-blue-400 fill-current" />
                      <span className="font-semibold text-blue-400">{batch.batch}</span>
                  </div>
                  <Link href={`/bb/${batch._id}`}><h3 className="text-xl font-semibold text-white mb-2 hover:text-emerald-400 truncate">{batch.name}</h3></Link>
                  <p className="text-sm text-white/60 mb-3">by {sellerName}</p>
                   <div className="flex items-center space-x-4 text-white/70 text-sm">
                      <div className="flex items-center space-x-1.5"><Heart className="w-4 h-4 text-red-400/70" /> <span>{batch.favorites || 0}</span></div>
                      <div className="flex items-center space-x-1.5"><Eye className="w-4 h-4" /> <span>{batch.views || 0}</span></div>
                  </div>
              </div>
               <div className="text-right flex flex-col items-end justify-between">
                  <div className="text-2xl font-bold text-emerald-400 mb-4">{convertPrice(batch.priceCNY || 0)}</div>
        ...
        <motion.div whileHover={{ y: -8 }} className="glass-morphism rounded-2xl overflow-hidden h-full flex flex-col">
          <Link href={`/bb/${batch._id}`} className="cursor-pointer">
            <div className="relative aspect-square"><BatchImage /></div>
          </Link>
          <div className="p-6 flex flex-col flex-grow">
            ...
            <Link href={`/bb/${batch._id}`}><h3 className="text-lg font-semibold text-white mb-2">{batch.name}</h3></Link>
            ...
            <div className="flex items-center justify-between mt-4">
                ...
                <div className="text-xl font-bold text-emerald-400">{convertPrice(batch.priceCNY || 0)}</div>
            </div>
