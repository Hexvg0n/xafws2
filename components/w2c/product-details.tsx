...
export function ProductDetails({ product }: { product: Product }) {
  ...
  const sanitizedDescriptionHTML = product.description ? DOMPurify.sanitize(product.description) : null;
  const allImages = [product.thumbnailUrl, ...product.mainImages].filter(Boolean) as string[];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          <div className="lg:col-span-3 space-y-4">
            <motion.div key={selectedImage} initial={{ opacity: 0.8 }} animate={{ opacity: 1 }} className="glass-morphism rounded-2xl p-2">
              <div className="aspect-square bg-white/5 rounded-xl overflow-hidden">
                <Image src={selectedImage} alt={product.name} width={800} height={800} className="w-full h-full object-cover" />
              </div>
            </motion.div>
            {allImages.length > 1 && (
          ...
          <div className="lg:col-span-2 space-y-6">
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="glass-morphism rounded-2xl p-8">
              ...
              <div className="text-xl font-medium text-white/70 mb-6">≈ {convertPrice(product.priceCNY)}</div>
              ...
              {product.availableSizes && product.availableSizes.length > 0 && (
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {product.availableSizes.map((size: string) => <Badge key={size} variant="secondary">{size}</Badge>)}
                  </div>
                </div>
              )}
