...
import { Product } from "@/lib/types"; // Importujemy typy
import { AddProductForm } from "./AddProductForm"; // Importujemy formularze
import { EditProductForm } from "./EditProductForm"; // Importujemy formularze
                ...
                {products.length > 0 ? products.map(product => (
                    <div key={product._id} className="flex items-center justify-between p-4 bg-white/5 rounded-lg">
                        ...
                        <div className="flex items-center gap-2">
                           <Button onClick={() => handleSendToDiscord(product._id)} size="sm" variant="ghost" title="Wyślij na Discorda" disabled={isNotifying === product._id}>
